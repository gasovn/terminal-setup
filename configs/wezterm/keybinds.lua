...
local scan_dirs = require 'scan-dirs'
local M = {}
...
function M.apply(config)
    ...
    config.keys = {
        -- ═══ Tabs ═══
        { key = 't', mods = 'CTRL|SHIFT', action = act.SpawnCommandInNewTab { cwd = wezterm.home_dir, domain = 'DefaultDomain' } },
        ...
        -- Opens an fzf picker over a fixed list of base directories
        -- (private/wezterm/scan-dirs.lua), independent of the current working
        -- directory. The picker script lists immediate subdirectories grouped by
        -- parent and opens nvim in the chosen one. See scan-dirs.example.lua.
        {
            key = 'e',
            mods = 'CTRL|SHIFT',
            action = act.SpawnCommandInNewTab {
                args = (function()
                    local args = { 'bash', wezterm.config_dir .. '/open-dir-in-nvim.sh' }
