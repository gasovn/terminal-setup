...
| Хоткей | Действие |
|--------|----------|
| `Ctrl+Shift+E` | fzf-пикер по подкаталогам заданных каталогов (`private/wezterm/scan-dirs.lua`) → nvim в новом табе; группировка по родителю, git-ветка в скобках |
...
## Каталоги проектов (Ctrl+Shift+E)

`Ctrl+Shift+E` открывает fzf-пикер по подкаталогам заранее заданных каталогов — независимо от текущего CWD. Список базовых каталогов лежит в приватном сабмодуле:

**`private/wezterm/scan-dirs.lua`** — список абсолютных путей:
```lua
return {
  '/home/user/code',
  '/home/user/projects',
...
Пикер показывает подкаталоги первого уровня каждого каталога, сгруппированные по родителю, с git-веткой в скобках. Поиск в fzf — `--exact`: слова через пробел работают как И, слово не дробится на буквы. Выбор открывает `nvim` в каталоге в новом табе. Формат — в `scan-dirs.example.lua`.
